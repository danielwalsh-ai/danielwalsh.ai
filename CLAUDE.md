...
- Cadence: hourly Make sweep posts at most 1 due Approved row per run (08:00–21:00); baseline one post/day. The scout schedules each new draft on its own day (index-spread) — never stack multiple posts on one date. **Viral exception (owner instruction, 01 Aug 2026):** a scouted story meeting the viral bar (broke <24h ago + major mainstream coverage + urgent money/hours/risk impact for UK SME owners; max 1/day) is auto-set to Approved with today's date and posts on the next hourly run WITHOUT approval; Daniel gets an "AUTO-POSTING (viral)" email and can revert the row to Draft within the hour to stop it. The "POST NOW" email link remains for manually fast-tracking normal drafts.
